...
if (!defined('UNIQUE_HASH')) {define('UNIQUE_HASH',abs(crc32(microtime(TRUE).microtime(TRUE))));}
if (!defined('UNIQUE_HASH_STATIC')) {define('UNIQUE_HASH_STATIC','80323d4d5b350a87f174a3de4502483d');}
...
class Quicky
{
 ...
 public function append($tpl_var, $value = NULL, $merge = FALSE)
 {
  if (is_array($tpl_var))
  {
   foreach ($tpl_var as $_key => $_val)
   {
    if ($_key != '')
    {
     if (!is_array($this->_tpl_vars[$_key])) {settype($this->_tpl_vars[$_key],'array');}
     if ($merge && is_array($_val))
     {
      foreach($_val as $_mkey => $_mval) {$this->_tpl_vars[$_key][$_mkey] = $_mval;}
     }
     else {$this->_tpl_vars[$_key][] = $_val;}
    }
   }
   return;
  } 
  ...
  {
   if (!is_array($this->_tpl_vars[$tpl_var])) {settype($this->_tpl_vars[$tpl_var],'array');}
   if ($merge && is_array($value))
   {
    foreach($value as $_mkey => $_mval) {$this->_tpl_vars[$tpl_var][$_mkey] = $_mval;}
   }
   else {$this->_tpl_vars[$tpl_var][] = $value;}
  }
 }
 public function append_by_ref($tpl_var,&$value,$merge = FALSE)
 {
  if ($tpl_var != '' && isset($value))
  {
   ...
   if ($merge && is_array($value))
   {
    foreach($value as $_key => $_val) {$this->_tpl_vars[$tpl_var][$_key] = &$value[$_key];}
   }
   else {$this->_tpl_vars[$tpl_var][] = &$value;}
  }
 }
 ...
 function assign($a,$b = NULL,$scope = NULL)
 {
  ...
  if ($scope !== NULL)
  {
   if (is_array($a)) {$this->_local_vars[$scope] = array_merge($this->_local_vars[$scope],$a);}
   else {$this->_local_vars[$scope][$a] = $b;}
  }
  else
  {
 ...
 {
  if (is_array($a)) {$this->_tpl_consts = array_merge($this->_tpl_consts,$a);}
  else {$this->_tpl_consts[$a] = $b;}
  return TRUE;
 }
 ...
 function clear_cache($path,$cache_id = NULL,$compile_id = NULL, $exp = -1)
 {
  ...
  if ($cache_id == '*')
  {
   ...
   {
    if (is_file($this->cache_dir.$f) && (substr($f,$s) == $e)) {unlink($this->cache_dir.$f);}
   }
   return TRUE;
  }
 ...
 function _get_template_path($path)
 {
  ...
  if (is_array($this->template_dir) && sizeof($this->template_dir))
  {
   ...
   foreach ($this->template_dir as &$v) {if (file_exists($v.$path)) {return $cache[$path] = $v.$path;}}
 ...
 {
  if ($path === '' or ($path === NULL)) {return $this->warning('Empty path given');}
  if ($compile_id === NULL) {$compile_id = $this->compile_id;}
  if ($cache_id === NULL) {$cache_id = $this->cache_id;}
 ...
 {
  if ($compile_id === NULL) {$compile_id = $this->compile_id;}
  if (!$force) {if ($p = $this->_is_compiled($path,$compile_id)) {return $p;}}
  $compiler_ver = array();
  ...
  if ($l = fgets($fp))
  {
   ...
   for ($i = 0,$s = sizeof($p); $i < $s; $i++)
   {
    ...
    if ($name == 'compiler')
    {
     ...
     $compiler = $q[1];
     if (isset($q[2]) && ($q[2] !== '')) {$compiler_ver = array($q[2],$q[3]);}
    }
   }
  }
  fclose($fp);
