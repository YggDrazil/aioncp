...
class sql_db {
	...
	// FIX old var is $persistency = false
	// thanks SoToNa
	...
	function sql_db($sqlserver, $sqluser, $sqlpassword, $database, $persistency = false) {
		if(ENCRYPT){
			...
			$sqlserver		=$enc->decode($sqlserver);
			$sqluser		=$enc->decode($sqluser);
			$sqlpassword	=$enc->decode($sqlpassword);
			$database		=$enc->decode($database);
		}
		
			
		$this->db_connect_id = ($persistency) ? @mysql_pconnect($sqlserver, $sqluser, $sqlpassword) : @mysql_connect($sqlserver, $sqluser, $sqlpassword) or die("Не удалось подключиться к $sqlserver");
		if ($this->db_connect_id) {
			if ($database != "" && !@mysql_select_db($database)) {
				@mysql_close($this->db_connect_id);
				$this->db_connect_id = false;
				die("Не удалось подключиться к базе $database");
			}
			// minifix
	...
	function sql_query($query = "", $transaction = false) {
		...
		if (isset($this->query_result) && $this->query_result) {
			...
			return FALSE;//($transaction == END_TRANSACTION) ? true : false;
		}
	}
